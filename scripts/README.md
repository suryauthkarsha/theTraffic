...
| `tests/` | `python3 -m unittest discover -s scripts/tests -v` — synthetic two-panel PDF parsing, junction matching, duplicates, conflicts, stale documents, weak matches, no-auto-verification. `bun test scripts/tests` — CSV cross-check rules. |
